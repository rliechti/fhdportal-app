    ...
    <div class="Datasets">
        <v-sheet min-height="70vh" rounded="lg">
            ...
            <v-dialog v-model="modalDga.status" max-width="800">
                <modal-dac-form
                    :dataset_id="modalDga.dataset_id"
                    :policy_id="modalDga.policy_id"
                    :form="modalDga.form"
                    @closeDacModal="modalDga.status = false"
                    :readonly="false"
                ></modal-dac-form>
            </v-dialog>
                ...
                <div v-else>
                    <div v-if="study_public_id">
                            ...
                            <template #item.actions="{ item }">
                                <p class="text-center" style="white-space: nowrap">
                                    ...
                                    <v-dialog v-model="modalDaa.status" max-width="800">
                                        <modal-policy-form
                                            :dataset_id="modalDaa.dataset_id"
                                            :policy_id="modalDaa.policy_id"
                                            :form="modalDaa.form"
                                            @closePolicyModal="modalDaa.status = false"
                                            :readonly="true"
                                        ></modal-policy-form>
                                    </v-dialog>
                                </p>
                        ...
                        <div v-else class="text-center pt-2">
                            Group related samples, runs, and analyses into a dataset for controlled
                            access and distribution.<br />
                            A dataset represents the collection of data that authorized users will
                            be able to request and access
                        ...
                        >
                            <template v-if="selectedDatasets.length">
                                ...
                                <template v-if="deleteDataset.status">
                                    <v-btn
                                        ...
                                        >confirm deletion of {{ selectedDatasets.length }}
                                        <span v-if="selectedDatasets.length > 1">Datasets</span
                                        ><span v-else>Dataset</span></v-btn
                                    >
                                    <v-btn
                            ...
                            <h4 class="text-center text-warning" v-else-if="!allDatasetsWithPolicy">
                        ...
                        <div
                            ...
                            v-else-if="study.status_type_id === 'SUB'"
                        >
                            Submission under review by the Data Access Commitee...
                        </div>
                    </div>
                </div>
            </v-container>
...
export default defineComponent({
    ...
    computed: {
        ...
        isReadyForSubmission() {
            ...
            if (this.study.status_type_id !== 'DRA' && this.study.status_type_id !== 'REV')
                return false
            ...
            schemaStore.getSchemas().then((schemas) => {
                _.forEach(datasetTypes, (datasetType) => {
                    if (schemas[datasetType] !== undefined) {
                        if (schemas[datasetType].ui_schema.displayedElements !== undefined) {
                            _.forEach(schemas[datasetType].ui_schema.displayedElements, (d) => {
