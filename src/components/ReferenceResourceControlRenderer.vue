...
import { VAutocomplete, VChip, VAlert } from 'vuetify/components'
...
interface ResourceStore {
    getStudySamples?: (params: { study_id: string }) => Promise<any>
    getStudyExperiments?: (params: { study_id: string }) => Promise<any>
    getStudyAnalysisFiles?: (params: { study_id: string }) => Promise<any>
    getStudyFiles?: (params: { study_id: string }) => Promise<any>
    samples?: any[]
    experiments?: any[]
    study?: {
        analysis_files?: any[]
...
export default defineComponent({
    ...
    setup(props: RendererProps<ControlElement>) {
        ...
        const schema = input.control.value.schema as Record<string, any>
        const resourceType = ref<string>((schema['x-check'] as string) || '')

        const resourceTypeTitle = ref<string>((schema['x-check_title'] as string) || 'title')
        const resourceTypeValue = ref<string>((schema['x-check_value'] as string) || 'public_id')
        const errorMessage = ref<string>('')
        const resourceName = ref<string>('')
        const resources = ref<Array<Record<string, unknown>>>([])
        const resourceStore = ref<any>({} as ResourceStore)
    ...
    computed: {
        ...
        sortedResources(): Array<Record<string, unknown>> {
            return this.resourceTypeTitle
                ? _.orderBy(this.resources, (r) => this.deepGetByPath(r, this.resourceTypeTitle))
                : this.resources
        }
    }
})
